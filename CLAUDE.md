...
| `heirlooms.py` | **Heirloom & Legacy Artifacts.** Named items (weapons, armor, tools, banners, memorials) created from masterwork crafting or faction events. Accumulate legend through battle, defense, heroic death, disaster survival. At legend threshold, items become faction relics granting cohesion bonuses. Inheritance chain: partner → child → faction leader → vault. Warfare looting (30% chance, relics protected). Defs in `defs/core/heirlooms.json`. |
| `governance.py` | **Governance & Edicts.** Faction leaders autonomously issue edicts (conscription, rationing, open/closed borders, festivals, martial law, research mandates, etc.) based on doctrine affinity, leader personality, and situational preconditions. One edict per faction at a time. Effects modify combat power, food consumption, cohesion, migration, build speed, disease recovery. 10 EdictDefs with moodlets, episodic memory, EventBus events. Defs in `defs/core/edicts.json`. |
| `feuds.py` | **Feuds & Grudges.** Persistent interpersonal conflict between individual Praxans. 7 GrudgeDefs (insult, argument, heirloom theft, betrayal, rivalry, leadership, kin-slayer) accumulate directed grudge weight with personality-weighted decay. 4 FeudStageDefs (Simmering→Open Hostility→Vendetta→Blood Feud) with escalating effects: moodlets, opinion drift, social avoidance, insult-chance boosts, cohesion/diplomacy penalties, ally recruitment, physical confrontation. EventBus auto-subscription (PERSONAL/WARFARE/MIGRATION). Defs in `defs/core/feuds.json`. |
| `chronicle.py` | **Legends & Chronicle.** Persistent historical narrative layer. Subscribes to all EventBus categories (drama ≥ 2). Derives Historical Eras (8 EraDefs: Age of Founding/Golden Age/War/Plague/Schism/Exploration/Rebirth/Legends) from rolling 5-min event window. Tracks per-Praxan Legendary Deeds (10 types: War Hero, Great Builder, Plague Survivor, etc.) with automatic detection from event logs. Legend Tiers (Notable 5pts / Renowned 12pts / Legendary 20pts) with moodlets, reputation events, episodic memories. First-tier assignment is silent; subsequent upgrades fire events. Era shown in HUD subtitle; legend status shown in inspect drawer overview. Snapshot serialized. Defs in `defs/core/chronicle.json`. |
...
| `moods.json` | `MoodDef` — AteRawFood, AteFineFood, Catharsis, FoughtOffInfection, + 7 social moodlets + 6 ritual moodlets + 6 reputation moodlets + 4 aspiration moodlets + 16 warfare moodlets + 9 cascade moodlets + 10 tradition moodlets + 16 migration moodlets (DefectorAnxiety, WarRefugee, FamilyReunion, etc.) + 5 era moodlets (EraOfProsperity, EraOfConflict, EraOfHardship, EraOfUncertainty, EraOfLegends) + 3 legend tier moodlets (NotableFigure, RenownedFigure, LegendaryFigure) |
...
| `heirlooms.json` | `HeirloomTypeDef` — 5 heirloom types (named_weapon, named_armor, founders_banner, masterwork_tool, memorial_stone) with inheritance priority, legend thresholds, stat bonuses + `HeirloomEventDef` — 10 legend-building events |
| `edicts.json` | `EdictDef` — 10 faction edicts (conscription, rationing, open/closed borders, research mandate, festival decree, martial law, construction drive, healing initiative, foraging mandate) with doctrine affinity, personality weights, effects, preconditions, moodlets |
| `feuds.json` | `GrudgeDef` — 7 grudge types (insult, argument, heirloom theft, betrayal, rivalry, leadership, kin-slayer) with weight, decay rate, personality weights + `FeudStageDef` — 4 escalation stages (simmering/hostile/vendetta/blood_feud) with moodlets, opinion drift, cohesion/diplomacy penalties, violence chance |
| `chronicle.json` | `EraDef` — 8 historical eras (founding, golden, war, plague, schism, exploration, rebirth, legends) with dominant event categories, thresholds, priority, moodlets + `LegendaryDeedDef` — 10 deed types (great_builder, war_hero, plague_survivor, master_artisan, great_healer, peacemaker, great_mentor, disaster_survivor, colony_founder, beloved_leader) with required event counts, legend points, reputation bonuses + `LegendTierDef` — 3 tiers (Notable 5pts, Renowned 12pts, Legendary 20pts) with moodlets |
| `memory_events.json` | `MemoryEventDef` — episodic memory types including `legendary_deed` and `legend_status` |
