...
| `moods.json` | `MoodDef` — AteRawFood, AteFineFood, Catharsis, FoughtOffInfection, + 7 social moodlets + 6 ritual moodlets + 6 reputation moodlets + 4 aspiration moodlets + 16 warfare moodlets + 9 cascade moodlets (CollectiveTrauma, EpidemicFear, HardenedByCrisis, NewLifeJoy, etc.) |
| `reputation.json` | `ReputationEventDef` — 29 event types (built_structure, crafted_masterwork, combat_victory, defended_colony, raid_kill, led_defense, fled_battle, etc.) + `ReputationTierDef` — 5 tiers (Luminary/Respected/Established/Marginal/Outcast) with moodlets |
